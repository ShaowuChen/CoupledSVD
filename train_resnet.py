...
# flags.DEFINE_string('model', 'resnet18', 'model name')
# flags.DEFINE_string('dataset', 'imagenet', 'imagenet/cifar10/cifar100')

# flags.DEFINE_string('root_path', '/home/test01/sambashare/sdd/Coupled_Pruning', 'root_path' )
...
# ckpt_path = FLAGS.root_path +'/'+FLAGS.dataset+'/'+FLAGS.ckpt_path+'/'+FLAGS.model
# os.makedirs(ckpt_path, exist_ok=True)
# log_path = ckpt_path+ '/'+FLAGS.dataset+'_'+FLAGS.model+'.log'


# # y_real=y-1 if FLAGS.dataset=='imagenet' else y 
...
# elif FLAGS.dataset=='cifar10' or 'cifar100':
...
# # '''Note: in imagenet, the label start from 1; so it should be substracted by 1'''
# # y_real=y-1 if FLAGS.dataset=='imagenet' else y 
# # is_training = tf.placeholder(tf.bool, name = 'is_training')

# # lr = tf.placeholder(tf.float32, name='learning_rate')

# value_dict = pytorch_pth2tensorflow_npy.convert( FLAGS.model)  
# print('start building network')
# #dynamic calling the function
# logits, prediction = eval('resnet.'+ FLAGS.model)(x, FLAGS.dataset, is_training, weight_dict=value_dict)
...
# # loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=y_real), name='loss')
...
#         if i % max(int(num_iterations//10),1) == 0:
#             print('i=',i)

#         # image_batch, label_batch = sess.run([val_images, val_labels])
#         # top_1, top_5 = sess.run([Top1, Top5], feed_dict={x: image_batch, y: label_batch, is_training: False})
...
#         print('epoch=',epoch)
#         print('j=',j)
#         log(['change_lr=%s, lr_decay=%s\n'%(FLAGS.change_lr, FLAGS.lr_decay)])

#         num_lr = num_lr/FLAGS.lr_decay if j in eval(FLAGS.change_lr) else num_lr
...
#             # summary_str, _, loss_eval = sess.run([merged_summary_op, train_op, loss], feed_dict={x: image_batch, y: label_batch, is_training: True, lr: num_lr})
...
#             if i%(max(int(num_iterations//50),1))==0:#print & add summary  51 times
#                 print(i)
#                 summary_wirter.add_summary(summary_str, i+num_iterations*j*FLAGS.batch_size)

#         end=time.time()
#         running_time_one_epoch = end - start

#         start = time.time()
#         top1, top5 = validate(sess)
#         end = time.time()
#         eval_time_one_epoch = end - start

#         informations = ["Epoch"+str(j)+" Top1:"+str(top1)+' Top5:'+str(top5), 'loss='+str(ave_loss), 'Runing_Time_1epoch:%f m, eval_time_one_epoch:%f s\n'%(running_time_one_epoch/60,eval_time_one_epoch)]
#         log(informations)
...
#             raw_top1, raw_top5 = validate(sess)
#             informations = ["raw Top1:"+str(raw_top1)+' Top5:'+str(raw_top5)+'\n']
#             log(informations)
